...
export default function Header() {
  ...
  const NavLink = ({ href, label, external = false }) => {
    ...
    const base =
      "relative px-3 py-2 text-sm font-medium transition-colors duration-300 hover:text-blue-400 after:absolute after:left-1/2 after:-bottom-px after:h-[2px] after:w-0 after:rounded-full after:bg-gradient-to-r after:from-blue-500 after:to-purple-500 after:transition-all after:duration-300 hover:after:w-full hover:after:left-0";
    const active = isActive ? "text-blue-400 after:w-full after:left-0" : "text-gray-300";
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-md bg-[#0b1120]/60 border-b border-white/10">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6 lg:px-8">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          <div className="relative group">
            ...
            <ul className="absolute left-0 mt-3 w-52 rounded-xl bg-zinc-900/95 border border-gray-700 shadow-lg opacity-0 scale-95 invisible group-hover:visible group-hover:opacity-100 group-hover:scale-100 transition-all duration-200 divide-y divide-gray-700 z-50">
              {iotLinks.map(link => (
                <li key={link.href}>
                  <NavLink {...link} />
                </li>
      ...
      >
        <ul className="space-y-3 px-6 py-4 border-t border-gray-700 bg-[#0b1120]/90 backdrop-blur-md">
          <li>
            <button
            ...
            >
              IOT
              <svg className={`h-4 w-4 transform transition-transform ${mobileIotOpen ? "rotate-180" : ""}`} stroke="currentColor" fill="none" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <ul className={`mt-2 pl-4 border-l border-gray-600 text-sm space-y-2 transition-all ${mobileIotOpen ? "max-h-96" : "max-h-0 overflow-hidden"}`}>
              {iotLinks.map(link => (
                <li key={link.href}>
                  <NavLink {...link} />
                </li>
